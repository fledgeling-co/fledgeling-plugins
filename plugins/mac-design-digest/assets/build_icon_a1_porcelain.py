#!/usr/bin/env python3
"""Build take A1 of the mac-design-digest icon — "The Porcelain Plate".

This built the shipping master from 18 Aug 2026 until 19 Aug 2026, when the user
chose take A2 in its place. It is preserved under its own name rather than
deleted, because it is still a scored take on `audit.html` and because it is the
one take in the set whose category read is right: a sealed certificate rather
than a card. A2's build script is now `build_icon.py`, and this one no longer
writes any shipped file.
...
What it lost, measured on the renders in `audit-renders/` on 19 Aug: rubric #7,
because plate #F7F4ED against ground #E7E4DC is 1.16:1 from the constants and
1.07:1 off the render, so the object's mass separates by hairline and shadow
rather than by value; and rubric #10, because that same relationship dissolves
the moment the render is reduced to luminance. 10 / 12.

    python3 build_icon_a1_porcelain.py   # writes icon-A1-porcelain.svg beside this file
"""
...
if __name__ == "__main__":
    out = HERE / "icon-A1-porcelain.svg"
