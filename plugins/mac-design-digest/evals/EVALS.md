...
| The real 134-app corpus snapshot in daily use | **2 genuine defects found, exit 1**, with 88 provenance-marked rows and 15 canon rules examined, no false failures |
...
- **`examined=0` is reported as a check that did not run**, never as a pass. On the real snapshot
  the lineage check had no app profiles to read, so it said so, reporting "this check found no material to
