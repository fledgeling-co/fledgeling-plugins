...
- **O — technical PRD, overview & marketing features**: create an initial comprehensive technical `PRD.md` file and `OVERVIEW.md` in the project root. `PRD.md` must cover all of the feature briefs in `docs/features-to-triage/` with full traceability (a complete requirements matrix mapping each brief in `docs/features-to-triage/*.md` to user stories, acceptance criteria, target platforms, data contracts, and status partitioned as Triaged/Backlog/In Progress/Built). `OVERVIEW.md` defines the mission, system architecture (with a Mermaid diagram), tech stack, module boundaries, and surfaces. Also fill `docs/MARKETING-FEATURES.md` (feature→benefit→proof rows for what the scaffold already ships, e.g. sign-in; pricing recommendation grounded in the research's competitor pricing, marked as recommendation until the owner confirms). Research-derived claims cite their report inline. Both files carry keep-current rules in CLAUDE.md.
- **D — design**: design-craft + ux-craft mock every surface, flow, menu, modal, and empty/loading/error state into `design/mocks/html/` (inventory first in INDEX.md); mac-design-studio for native app design. Trawl real shipped UI before committing a direction — design-craft's `plugins/design-craft/skills/design-craft/references/mobbin-trawl.md`, two or three aimed searches per surface family, with the took/left ledger in INDEX.md; a direction derived only from memory is what "boring and uninspiring" names when it comes back.

  **The app icon goes to `create-mac-icon:create-mac-icon`, whole.** Invoke that skill (or brief an agent to read its SKILL.md and follow it) and let it own the commission end to end: direction from its corpus catalogue, subject-mined glyph, all three engines, the fidelity loop that scores the shipped SVG against the winning raster, and its `audit.html` written from `assets/icon-audit-template.html`. Do not substitute a bare `media-gen-pro` call plus a hand-rolled contact sheet — that path skips the corpus, the rubric, the loop, and every lesson the skill has accumulated, and the icons come back, in the user's words, "really basic compared to all of the macos icons". `media-gen-pro` with `svg: true` is Engine B *inside* that pipeline, not a replacement for it.

  Icon deliverables into `design/icon/`: the layered SVG master and its build script, the alternates, the retina renders, and `audit.html` at 128 / 64 / 48 / 32 / 16 css px plus tinted and silhouette. Gate it with `python3 <create-mac-icon>/skills/create-mac-icon/scripts/audit_sheet.py check design/icon` — it must exit 0 — then open the sheet and read it. Every icon in the set shares one outer silhouette from `squircle-path.txt`.

  If `create-mac-icon:create-mac-icon` is not installed, say so plainly in the phase report and fall back to `mac-design-studio:mac-design-studio`'s icon section; note that the master then ships unmeasured against any reference.
- **M — marketing site**: `design/marketing/index.html` — create-luke-content writes every word (lint it), design-craft + ux-craft build the premium single page against the researched quality bar in the reference (zero Krebs slop tells; LCP ≤2.5s with three.js out of the critical path; native scroll; motion at one or two authored moments; an interactive mock slice from phase D; pricing per the evidence table; login/signup to `/login`), media-gen-pro supplies imagery into `design/marketing/assets/`; gate with design-review. Trawl reference sections first (`search_sections`, `platform: web`) — hero, pricing, footer — and record the ledger; "it doesn't really feel like a marketing website and the layouts are terrible, did you use mobbin mcp for inspo?" is the failure this step exists to avoid. Serve the page and open it at 1440 and 390 before calling it done. Porting it into apps/web becomes a P0 brief.
...
- Register the project in the portfolio: run the `armada-sync:armada-sync` skill so `~/Dev/ARMADA.md` gets its entry.
- Point at the first-feature route: drop a brief in `docs/features-to-triage/` and run `ship-feature:ship-feature`, or route it via `ship-armada:ship-armada`.
