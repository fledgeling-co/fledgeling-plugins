...
if [ -n "$SKILLS" ]; then
  ...
  for s in "${LIST[@]}"; do
    ...
    if [ "$qualified" -eq 0 ] && printf '%s\n' $BUILTIN_BLOCKED | grep -qx "$name"; then
      bad "skill /$s" "built-in or disable-model-invocation — a scheduled fire delivers it as PLAIN TEXT. Call it from the main turn, or name the plugin-qualified skill / the underlying command."
      continue
    fi
