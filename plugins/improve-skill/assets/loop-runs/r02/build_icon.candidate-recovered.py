...
PIT_ELEV = 50.0
FIBRE_AZ = frame_azimuth(UX, UY, NX, NY)                    # 102 deg: the key, in blade
PIT_AZ = frame_azimuth(UX, UY - K_RISE, NX, NY)             # the key, in the top face
...
def fibre_ramp_stops():
    ...
    The far stops were re-cut once after the first render: at 0.34/0.24 the near-key band
    came out 9.15 hp / 0.511 edge density against C2's 5.58 / 0.142, because the relief
    is a MULTIPLY and a base that bright amplifies whatever it is given."""
    span = LY_MAX - LY_MIN
    def off(ly):
        return (ly - LY_MIN) / span
    stops = [(0.0, 0.05), (off(0.0), 0.12), (off(0.0), 0.80),
             (off(285.0), 1.00), (off(513.0), 0.20), (1.0, 0.14)]
    return "\n    ".join(
...
def shaving():
    ...
    # ROUND 8 - four of these read as four drawn lines at 1024; C2's curl reads as fibre,
    # 25% of its pixels above a gradient of 4/255 against our 5-12%. The answer is more
    # of the same construction, not a different one: eighteen irregularly spaced copies,
    # alternating a dark fibre and the pale one beside it, each faint enough that no
    # single line is findable. They stay on the ROLL only - past TAIL_N the ribbon's
    # opacity taper is doing the work of letting the ground show through, and an opaque
    # stroke laid over it would fill the taper back in.
    grain_lines = []
    roll = near[TAIL_N:]
    for i in range(18):
        k = 0.055 + 0.052 * i + 0.016 * rnd()          # irregular, never a comb
        dark = i % 2 == 0
        op = (0.020 + 0.013 * rnd()) if dark else (0.014 + 0.010 * rnd())
        chain = [(x + SX * k, y + SY * k) for x, y in roll]
        grain_lines.append(
            f'<path d="{open_poly(chain)}" stroke="{"#7E6E56" if dark else "#FFF3DF"}" '
            f'stroke-opacity="{op:.3f}" stroke-width="{1.0 + 0.5 * rnd():.2f}"/>')
...
{SHAVING_GRAD}  <!-- ROUND 7: re-centred toward the key and deepened. C2's corners measure TL 0.869,
  ...
  <!-- ROUND 8, the fibre. Both filters below are declared in a LOCAL frame and applied
       inside a <g> carrying that frame, so the noise is anisotropic along the direction
       of travel without a single rotated path being authored. Their lights are the
       scene's one key re-expressed in that frame ({FIBRE_AZ:.0f}deg / {PIT_AZ:.0f}deg),
       not a convenient 225 - a relief lit from its own imaginary source is the fastest
       way to lose the single-light-model check on the rubric. -->
{relief_filter("fibreRelief", FIBRE_BF, FIBRE_SCALE, FIBRE_ELEV, FIBRE_AZ, 7)}
{relief_filter("pitRelief", PIT_BF, PIT_SCALE, PIT_ELEV, PIT_AZ, 11)}
  <!-- how much fibre survives, along the local y that runs away from the cut -->
  <linearGradient id="fibreFall" gradientUnits="userSpaceOnUse"
                  x1="{AX + NX * LY_MIN:.1f}" y1="{AY + NY * LY_MIN:.1f}"
                  x2="{AX + NX * LY_MAX:.1f}" y2="{AY + NY * LY_MAX:.1f}">
    {fibre_ramp_stops()}
  </linearGradient>
  <mask id="fibreMask" maskUnits="userSpaceOnUse" x="0" y="0" width="{W}" height="{W}">
    <rect width="{W}" height="{W}" fill="url(#fibreFall)"/>
  </mask>
</defs>
...
<g clip-path="url(#tileMask)">
  ...
  <g id="bg">
    ...
    <!-- ROUND 8: the fibre, over BOTH fields at once. It is one noise field masked by
         one ramp, so a fibre that runs up to the cut continues past it at a tenth of its
         height - torn above, ghosted below. That is the icon's claim (one surface, two
         states) stated in the material rather than argued by colour. Normalised on the
         flat value, so the field means move by ~1 grey level and polarity, figure-ground
         and the 16px read are untouched; measured after, not assumed. -->
    <g mask="url(#fibreMask)">
      <g transform="{MATRIX}" filter="url(#fibreRelief)"><g transform="{MATRIX_INV}">
        <path d="M0 0 L{W} 0 L{W} {B_RIGHT:.1f} L0 {B_LEFT:.1f} Z" fill="url(#roughField)"/>
        <path d="M0 {B_LEFT:.1f} L{W} {B_RIGHT:.1f} L{W} {W} L0 {W} Z" fill="url(#truedField)"/>
      </g></g>
    </g>
  ...
  <g id="fg">
    ...
    <!-- the plane iron as a real solid: a front face dropping to the ground, and a top
         ...
         ROUND 8: the same relief, isotropic and shallower, runs over the iron's own
         faces - cast stone pits, it does not have grain. It wraps the paths in place
         rather than duplicating them, and because the modulation is normalised the
         block's mean holds to within a quarter of a grey level, so the 5.27:1
         figure-ground the silhouette depends on is unchanged. -->
    <g transform="{MATRIX_TOP}" filter="url(#pitRelief)"><g transform="{MATRIX_TOP_INV}">
