...
2. **`research_plan`** (free) — pass the improvement question with a
   `decisionContext` naming the actual skill and the changes you want to
   be able to defend. The plan names the panel members and their cost
...
the subtopics (architecture, scoring, evaluation, prompt-level findings),
bound the time horizon to recent years, and exclude the adjacent fields
you don't need. A vague question buys a survey; a decision-shaped
question buys defensible design changes.
