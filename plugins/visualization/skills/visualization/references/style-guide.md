...
**Moved, and measured.** The five hand-picked "editorial tone" series colours
this file used to carry failed the perceptual gate in both modes — five of five
below the chroma floor, worst normal-vision ΔE 10.3 light / 9.1 dark against a
floor of 15. They read as five greys with a tint.

The validated replacement, its derivation, and the rules that come with it live
in [`series-palette.md`](series-palette.md). Slots are assigned in fixed order,
`accent` stays reserved for the focal series, and any brand substitution is
re-validated against that brand's own surfaces.

**Don't backfill series colours to non-chart types.** Architecture, swimlane and
the rest continue to use muted-ink variants. The series palette is for diagrams
where multiple entities must be told apart, not a licence to add colour.
