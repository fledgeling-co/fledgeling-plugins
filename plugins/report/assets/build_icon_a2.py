#!/usr/bin/env python3
"""Engine A, second take — "The Fold, inverted". Not the shipped master.

Written on 19 Aug 2026 because Engine B (Arrow) refused the commission on a credit
balance, and the pipeline's floor is three takes made by real engines rather than
three variations dressed as three. This is the widened Engine A the skill's own
fallback clause calls for, and it is a genuine alternative rather than a repaint:
it keeps the direction and the silhouette and inverts the value architecture the
shipping master loses two rubric points on.
...
separated into two stepped leaves. What changes:
  ...
  * the sheet is warm graphite instead of porcelain, so the object is the dark
    ...
    master measures 1.04:1 paper-to-ground and 1.96:1 ink-to-paper; this measures
    8.4-13.2:1 sheet-to-ground and 7.4-12.5:1 ruling-to-sheet.
...
counts them. Geometry is deliberately identical to `build_icon.py` so the two takes
differ on value and material only — that is the comparison the audit sheet makes.
"""
...
OUT = "icon-A2-fold-inverted.svg"

# ── geometry: lifted verbatim from build_icon.py, so nothing but value moves ────────
S = 1024
...
def build() -> str:
    ...
    for i in range(2):
        ...
        # One contact shadow per leaf, not one slab behind both. build_icon.py casts a
        # single blurred rect spanning LEAF_H*2 + LEAF_GAP, so its middle shows through
        # the gap between the leaves as a grey panel floating on the porcelain — visible
        # in the master's own 1024 render. Per-leaf shadows put the darkness only where
        # a leaf actually rests on something.
        leaf_shadows.append(
...
def main() -> int:
    ...
    print(f"wrote {OUT} — a losing take kept for the audit sheet; it does not ship, "
          f"so nothing here rasterises icon.png")
    return 0
