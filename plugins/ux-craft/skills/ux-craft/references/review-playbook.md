...
- **When you cannot render, say so in the report — do not degrade silently.** This is the branch that gets skipped, and it is the one with a measured cost: one recorded run named a browser engine that failed on all four attempts, never ran, and shipped a five-row review matrix reading PASS on every row. So: if the page will not render, the review continues from source and the verdict block says which checks were therefore not run, by name. A source-only review is a legitimate deliverable; a source-only review presented as a rendered one is not. **If you did not run a probe, do not write a review section that implies you did** — the three closing lines (Built / Measured / Not checked) are how you say what actually happened, and the "not checked" list names actual omissions; it may be empty when the requested scope was exercised.

### The engine, and what it cannot see

**Choose a currently available, permitted browser.** The commands and limits below describe Obscura in the original August 2026 environment; they do not prohibit other tools or establish current behavior. Read the relevant tool interface before using these examples. `obscura fetch <url> --screenshot page.png`, `--eval "<expr>"`, `--dump text|html|markdown|links`; `obscura serve --port <n>` for CDP when a driver is needed. **Localhost is blocked by default** — anything on `127.0.0.1`, a `192.168.*` host or a local dev server needs `--allow-private-network` or it fails as an SSRF block that looks like a dead server.

These recorded blind spots produced false findings. When one reproduces in the current engine, report that criterion as **not checked**, or use a permitted engine that can measure it:
