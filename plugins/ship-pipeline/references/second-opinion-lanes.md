...
decisions through this file. It generalises the `clarify` skill's Gate 4 (one verified
Codex lane) into an ordered lane set plus a panel protocol, so the pipeline defers decisions to
models instead of stalling on a human — and so the deferral is evidence-shaped rather than vibes.
...
4. **A model can settle a technical question.** Which design holds up, whether an approach has a
   flaw, which library fits this codebase — the human is not the only thing that can answer these.
   Use a lane (below), or a panel for the genuinely open forks.
5. **What survives all four reaches the human**: taste, cost, scope, risk tolerance, and their
   systems — plus the standing override: anything destructive, irreversible, outward-facing, or
   costly gets asked regardless, even when a conventional default exists. In an attended session,
   ask via `AskUserQuestion` in clarify's shape (batched, ≤3 questions, an earned recommendation
   with its reason, consequence-not-mechanism wording). In an unattended run, never block: park
...
gives a verdict on *this* codebase.
...
# Lane C — Google family
perl -e 'alarm shift @ARGV; exec @ARGV' 600 \
  agy -p "<question + evidence>" > /tmp/so-<slug>-agy.md 2>/tmp/so-<slug>-agy.log

# Lane D — xAI family (harness fallback: cursor-agent)
perl -e 'alarm shift @ARGV; exec @ARGV' 600 grok -p "<question + evidence>" > /tmp/so-<slug>-grok.md 2>&1
# fallback: cursor-agent -p --force "<question + evidence>"
```
...
- **Verify the wire, not the flags.** The captured header lines are the evidence a lane ran as
  ...
  not a quiet pass.** A failed lane means you decide alone, and say so.
- **Bound every call** (`perl -e 'alarm …' 600`; no `timeout(1)` on macOS; codex has no timeout
...
3. **Swap the order** of options between members (position bias is non-random and model-specific).
4. **Tally without dropping.** A member that returned nothing is counted as NONE and reported —
   a silent exclusion turns a broken harness into a clean-looking result. Majority decides;
   report the split per family.
5. **Disagreement is the signal, not noise.** A split panel on a high-severity fork is exactly
...
6. **Record it**: one line in the artifact — `panel: B/C/D → 2-1 <option> (D dissented: <reason>)`
   — so the decision's provenance survives the session.

## Deep research (Dossier) — when the question is about the world

A fork that turns on external facts (market norms, a vendor's real behaviour, prior art) is a
research question, not an opinion question. Use the Dossier MCP: `research_plan` (free) →
`research_start` with no provider (assembles the free-CLI + paid panel), export and **read the
reports in full**, `research_verify_citations` before relying on findings, and write the report to
the repo's `docs/deep-research/<slug>.md` so the brief can point at it. Free lane first; paid
panel when the decision is load-bearing enough to justify the spend (say the estimated cost in the
ledger line). Support is counted in independent domains, never in how many backends agree.
