...
cd "$(dirname "$0")/.."          # plugin root (shipyard)
PLUGIN_ROOT="$PWD"
EVALS="${EVALS_FILE:-$PLUGIN_ROOT/evals/evals.json}"
...
# 4-way parallel: two eval-arm pairs in flight per slot wave
jobs_running=0
for id in $ids; do
