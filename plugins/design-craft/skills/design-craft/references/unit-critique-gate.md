...
**The UX axis.** When the unit is a flow step, a form, navigation, or an AI-facing surface, add a seventh score — `ux` — judged from the companion **ux-craft:ux-craft** skill's canon: the five states on every data surface (loading / empty / error / populated / edge), cognitive-load budget, recognition over recall, error recovery, and (for AI surfaces) disclosure and user control. Load the matching ux-craft:ux-craft reference (`flows-and-forms.md`, `ai-product-ux.md`, `review-playbook.md`) before judging; if ux-craft:ux-craft is not installed, judge from those principles anyway and say so.
...
- **Default — critique it yourself, in a deliberately separate pass.** Use the lint result and fresh crops to separate authoring from review, then read the *rendered* artifact top to bottom **as the reviewer**, scoring each axis against the anchors. Never critique from your memory of writing it — read what's on the screen, not what you meant to put there. This is a handful of tool calls; it does not need delegating.
...
Per-unit gates are **depth**; `polish-pass` is **breadth**. Reuse completed unit evidence in the final pass; assess only changed or cross-cutting concerns. The polish pass owns the cross-cutting axes a per-unit gate can't see: consistency *between* units (palette or type drift across pages, spacing-scale divergence), navigation and IA coherence, the deliverable-wide accessibility sweep, and the final subtractive look.
