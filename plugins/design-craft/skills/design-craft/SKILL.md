...
name: design-craft
description: >-
  Use whenever the user asks to design or build a user-facing visual artifact — a landing page, app screen, dashboard, clickable prototype, native iOS/Android mockups, wireframe, design variations, a tweakable panel, a motion piece or scroll-driven marketing page, a 3D/WebGL hero, a print document, generated imagery, or a design system — or to review/fix a design (accessibility audit, "looks AI-generated"/remove-the-slop, hierarchy check, interaction-states/motion pass, layout breakage, pre-ship polish, redesign/modernise an existing site). Triggers — "design a…", "build a UI/prototype", "animate this", "make this look intentional", "wireframe this flow", "polish before we ship", "redesign this page". Slides route to deck-craft:deck-craft; a DESIGN.md from screenshots or a URL routes to design-md-from-screenshots:design-md-from-screenshots / design-md-from-website:design-md-from-website. An opinionated, AI-slop-resistant designer that iterates — per-unit critique gates, 37 routed reference files, autonomous mode, and a deterministic lint that computes WCAG contrast from source, mirrors the capture engine's and the artifact CSP's real limits, proves every rule can fire, and prints what it did not check. Carries dated engine limitations so a current run distinguishes unavailable checks from successful measurements. Consecutive greenfield sites are a defect: aesthetic ideation runs `trawl:trawl` before the direction commits, and a lookalike gate scores the first viewport against this session, the category default, and a named neighbour. Pairs with ux-craft:ux-craft for flows, forms, UX review.
---
...
2. **Acquire design context.** Read the design system, brand guidelines, codebase, screenshots, or UI kits — whatever exists. Mocking from scratch is a last resort. **Then widen the world, then trawl shipped UI.** On greenfield hi-fi with a free aesthetic axis, run `references/aesthetic-ideation.md` — it invokes `trawl:trawl` at `--any` (or `--standard` after a generic rejection) so the direction is not one model talking to itself. Then trawl real shipped UI for the surface type via `references/mobbin-trawl.md`. Ideation supplies the world; Mobbin supplies density, sequence and the surfaces a spec never lists. A direction derived only from memory converges on the shape every model ships for the category, which is what "looks like every other app website" names when it comes back as feedback. Two or three aimed Mobbin searches, images opened rather than listed, and a short ledger of what you took and what you deliberately left — including the ideation `TOOK`/`LEFT` lines.
...
7. **An unterminated block swallows every rule after it, and the query still reports as matching.**
   One dropped closing brace on a media query is the cheapest possible edit and the hardest possible
   symptom: measured 21 Aug 2026, a patch lost the `}` on `@media (min-width:1180px)`,
   `matchMedia('(min-width:1180px)').matches` returned **true**, the `.block{display:grid}` rule was
   present and greppable in source, and the element computed `display: block`. Every source check
   passed except the only one that mattered. This is the list's own defence stated at its sharpest —
   the presence of a rule tells you nothing; read the computed value on the node. Where a patch is
   applied programmatically, count braces in the inserted text before writing it.
...
**Show work early and often** — surface the skeleton so the user catches misunderstandings while they're cheap. **Brief summaries** — lead with the delivered outcome and include material evidence, caveats and remaining work; the final response must stand on its own. **Do the looking yourself** — render, screenshot, probe the DOM, read the console after each substantive batch of visual changes; reuse that evidence for overlapping gates and repeat only when the artifact changes or a finding stays unresolved. **Honest progress** — if you can't verify a behavior (no browser, no test data, an unreachable dependency), say so. **Deliver the whole count** — a multi-unit brief (12 sections, 5 screens) locks its unit count up front; if early stopping is unavoidable, say "X of Y complete, resuming at Z" rather than silently compressing or dropping the remaining units. Keep the ledger where a fresh session can find it — the todo plus a comment in the artifact naming which units are done, which are drafted-but-ungated, and what the committed direction is — and don't wind a build down early to conserve room; save the state and keep going. **Written deliverables get calibrated, not filled** — a document, spec, or findings report covers what the task needs and stops.
...
Two more, **measured 21 Aug 2026** on a long-form page, both of which broke a whole layout while
reporting nothing: **`ch` units inside `minmax()` do not resolve** — `grid-template-columns:
minmax(0,68ch) minmax(0,25ch)` collapsed both tracks to about 100px and set the entire page one word
per line, with the media query matching and the rule present in source. Size grid tracks in `fr` plus
a pixel track and hold the prose measure with `max-width` on the text, which is where a measure
belongs anyway. And **`clip-path` is not applied**, which quietly disarms the popular full-bleed
idiom: `box-shadow: 0 0 0 100vmax <colour>` relies on `clip-path: inset(0 -100vmax)` to contain it, so
without the clip the shadow spreads in every direction and paints over the content *above* the band —
measured, it covered the last line of the preceding section. Use a plate or a negative-margin bleed
that needs no clipping, and prefer the technique that is correct in every engine over the one that is
correct in one.

**Report coverage from the current run.** `references/visual-verification.md` Phase 0 provides the historical table and capability probes. When a limitation reproduces, use another available, permitted tool or name that criterion as unverified. Reuse the captures and checks already collected; do not declare a limitation from a historical machine as if it were measured now.
...
| `references/aesthetic-ideation.md` | Before `frontend-aesthetic-direction.md` commits on greenfield hi-fi with a free aesthetic axis, and again when a build came back as generic. Invokes `trawl:trawl` so consecutive sites do not share a world. |
| `references/aesthetic-worlds.md` | When the range map needs candidates that are not this model's reflexes, or the user names an anchor ("Stripe Press", "Are.na", "Tufte"). Load the matching group, not the whole table, then at most two files from `references/aesthetic-worlds/`. |
| `references/diversity-ledger.md` | Before committing a greenfield visual direction, and after it is settled. Reads and records `<project>/.design-craft/diversity-ledger.json`. |
| `references/current-moves.md` | When a 2026 brief needs axes that are not the 2023–24 defaults. Dated permissions, not a costume checklist. |
...
| `references/make-a-deck.md` | Pointer for slide requests; route the full job to `deck-craft:deck-craft` rather than rebuilding a deck shell here. |
...
| `references/lookalike-gate.md` | After the first viewport is on disk, before `polish-pass`. Proves this page is not the last commission, the category default, or the named neighbour with the nouns swapped. |
| `references/polish-pass.md` | Before any delivery/ship. Runs the review lenses, then fixes. Hands over first on a large build. |

**Decks route out.** Any slide/presentation request goes to the `deck-craft:deck-craft` skill, which is self-contained and owns the whole deck flow; don't rebuild a deck shell here. `references/make-a-deck.md` is a pointer so older references still resolve.

**Chaining.** Greenfield: `discovery-questions → aesthetic-ideation (trawl:trawl) → mobbin-trawl → frontend-aesthetic-direction (aesthetic-worlds + current-moves as candidate sources) → wireframe → make-a-prototype → lookalike-gate → polish-pass`, with `unit-critique-gate` running per unit inside the build step and reading `visitor-modes.md` / `motion-design.md` / `depth-and-3d.md` / `laws-of-composition.md` as the build touches their territory. Brand-aware: `design-system-extract → generate-variations → make-tweakable → polish-pass`. Motion deliverable: `discovery-questions → make-an-animation → motion-design (review gate) → polish-pass`, escalating to `gsap-motion.md` when the piece needs choreographed or scroll-driven sequencing. Mobile app: `discovery-questions → mobbin-trawl (platform: ios) → mobile-design → make-a-prototype → polish-pass`. Redesign: `redesign (mode + audit) → mobbin-trawl where the brief wants divergence → modernisation levers → polish-pass`. Any Operate or Read surface reads `visitor-modes.md` before the direction is chosen.

**The ux-craft:ux-craft layer chains in alongside these** (ch. 16): flows and forms load the matching ux-craft:ux-craft reference before building; AI-facing surfaces load its AI-product-UX guidance; every `polish-pass` includes the UX lens.
...
- **Aesthetic ideation** uses `trawl:trawl` via `references/aesthetic-ideation.md` when that skill is installed. Not installed is a one-line note and the weaker verbalized-sampling substitute in `frontend-aesthetic-direction.md`, never a silent skip. Invoke it once per commission; do not stack extra subagents on top of the ones it already launches.
...
Designs that look intentional come from thinking that is intentional. Every choice has a reason. Every element earns its place. Every interaction gives feedback. Every detail is resolved or honestly placeholder'd. The user is your manager — show your work, ask before you assume, and deliver less but better.
