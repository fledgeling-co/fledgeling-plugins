...
# For the blind-mutation pass. Both lists come from the campaign config where the
# project declares them; these are only the fallback for a Rust/RPC shape.
...
def pass_blind(root: Path, mutators: tuple[str, ...], readers: tuple[str, ...]
    ...
    for f in files:
        ...
        for i, (pos, name) in enumerate(starts):
            ...
            last, which = -1, None
            for v in mutators:
                for m in re.finditer(re.escape(v) + r"\w*\s*\(", body):
                    if m.start() > last:
...
def main() -> int:
    ...
    ap.add_argument("--mutator", action="append", default=[],
                    help="A verb that changes state. Repeatable; replaces the defaults.")
    ap.add_argument("--reader", action="append", default=[],
                    help="A verb that reads state. Repeatable; replaces the defaults.")
    ...
    if args.tests:
        ...
        if not root.exists():
            ...
            muts = tuple(args.mutator) or DEFAULT_MUTATORS
            rds = tuple(args.reader) or DEFAULT_READERS
