...
**A blind quality panel** (`evals/blind-panel/`). Both versions' outputs for each problem were anonymised as Option A/B in seeded-random order, and judged by multiple model families with no access to the skill files, so no judge knew which output came from which version or what either was supposed to look like: a Claude-family judge in strict isolation, grok-4.5 at high effort, composer-2.5 as a bonus family, and a Codex gpt-5.6 leg that runs when its usage window resets. Multi-family blind panels are themselves an evidence-backed choice; single LLM judges disagree with each other often, and they did here too. The panel's verdict is more honest than flattering: v2 wins breadth and trap detection clearly, the original won actionability on some problems by phrasing first steps in the problem's native toolchain, and the FOCUS instruction was changed the same day to require exactly that. Per-eval verdicts, the un-blinding map, and the raw judge reasoning are all in the repo.
