...
**`grade.py` cannot be run as shipped.** It reads `out-old/target.findings.json` and `out-new/target.findings.json` from the current directory, and neither directory exists anywhere in the repository. It was written to compare two versions of the harness, and the run output it compares was never committed. To get the table above I created both paths as symlinks to the one capture I had, and read only the current-harness column. The tool works; it just has no documented way to invoke it and no fixture wiring, so a later reader has to reconstruct the setup, as I did.
...
- **The one real defect in the answer key is not fixed.** `grade.py` still has no documented invocation. Naming it here is the whole of what this file does about it.
