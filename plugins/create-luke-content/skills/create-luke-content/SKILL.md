...
- **No noir case-file register.** Luke names the actor and the number in anything he reports happened, not only in bad news: "I recorded a merge that had not happened", not "a merge that had never happened"; "the judge reads 1,460 pairs in eight minutes", not "the reading lane is the slow part". Three shapes make that register and they compound: an event named by its absence, a passive or abstract subject where an actor belongs, and an ironic reversal closing the clause. A verbless noun phrase does the same thing ("Markers with nothing holding them"), so write the finding as a sentence carrying what happened and what it cost. It reads as a film trailer, and it withholds exactly what a reader came for; `references/ai-writing-signs.md` §1.8 carries the measurement.
