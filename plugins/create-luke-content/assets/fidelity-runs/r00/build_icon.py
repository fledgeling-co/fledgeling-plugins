...
EMBER_TOP, EMBER_MID, EMBER_BOT = "#FA9255", "#E4652E", "#B0340D"
EMBER_WALL_TOP, EMBER_WALL_BOT = "#8E2A0B", "#5C1704"
EMBER_CORE = "#FFD8AB"
EMBER_EDGE = "#7E2308"
EMBER_RULE = "#681A05"
EMBER_GLOW = "#F79A61"
...
LINE_TOP, LINE_MID, LINE_BOT = "#52473A", "#3A3024", "#241C13"
LINE_RIM = "#8E8070"
...
def make_ticks(x_start: float, x_end: float, y_base: float, tick_len_major: float, tick_len_minor: float, step: float = 16) -> str:
    ...
    for i in range(n + 1):
        ...
        w = 3.5 if is_major else (2.4 if is_mid else 1.8)
...
def svg() -> str:
    ...
    measuring_wall = rr(MX, MY + 16, MW, MH, MR)
    prose_sheet = rr(SX, SY, SW, SH, SR)
    prose_wall = rr(SX, SY + 12, SW, SH, SR)
  ...
  <defs>
    ...
    <linearGradient id="emberPlate" x1="{MX + MW * .15}" y1="{MY}" x2="{MX + MW * .75}" y2="{MY + MH}"
      ...
      <stop offset=".45" stop-color="{EMBER_MID}"/>
    ...
    <linearGradient id="emberRim" x1="{MX}" y1="{MY}" x2="{MX + MW}" y2="{MY + MH}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{EMBER_CORE}" stop-opacity=".85"/>
      <stop offset=".45" stop-color="{EMBER_GLOW}" stop-opacity=".55"/>
      <stop offset="1" stop-color="{EMBER_EDGE}" stop-opacity=".75"/>
    </linearGradient>
    <linearGradient id="emberInnerGlow" x1="{MX}" y1="{MY}" x2="{MX}" y2="{MY + 140}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{EMBER_CORE}" stop-opacity=".42"/>
    ...
    <linearGradient id="emberSideShade" x1="{MX + MW * .5}" y1="0" x2="{MX + MW}" y2="0"
      ...
      <stop offset="1" stop-color="{SHADOW_DEEP}" stop-opacity=".24"/>
    </linearGradient>

    <!-- Upper plane: translucent frosted prose sheet gradients -->
    <linearGradient id="frostFace" x1="{SX + SW * .15}" y1="{SY}" x2="{SX + SW * .70}" y2="{SY + SH}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{FROST_TOP}" stop-opacity=".92"/>
      <stop offset=".40" stop-color="{FROST_MID}" stop-opacity=".84"/>
      <stop offset="1" stop-color="{FROST_BOT}" stop-opacity=".78"/>
    </linearGradient>
    <linearGradient id="frostWall" x1="{SX}" y1="{SY}" x2="{SX + SW}" y2="{SY + SH}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{FROST_WALL}" stop-opacity=".85"/>
      <stop offset="1" stop-color="{FROST_SHADOW}" stop-opacity=".95"/>
    </linearGradient>
    <!-- Warm bounce from ember plate bleeding into lower-left of frost sheet -->
    <linearGradient id="frostEmberBleed" x1="{SX}" y1="{SY + SH * .25}" x2="{SX + SW * .5}" y2="{SY + SH}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{EMBER_GLOW}" stop-opacity="0"/>
      <stop offset=".50" stop-color="{EMBER_GLOW}" stop-opacity=".16"/>
      <stop offset="1" stop-color="{EMBER_MID}" stop-opacity=".32"/>
    </linearGradient>
    <linearGradient id="frostRim" x1="{SX}" y1="{SY}" x2="{SX + SW}" y2="{SY + SH}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{FROST_RIM}" stop-opacity=".95"/>
      <stop offset=".50" stop-color="{FROST_RIM}" stop-opacity=".50"/>
      <stop offset="1" stop-color="{FROST_SHADOW}" stop-opacity=".40"/>
    </linearGradient>
    <linearGradient id="frostSatin" x1="0" y1="{SY}" x2="0" y2="{SY + SH * .45}"
                    gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{FROST_RIM}" stop-opacity=".28"/>
      <stop offset="1" stop-color="{FROST_RIM}" stop-opacity="0"/>
    </linearGradient>
    <radialGradient id="frostAO" cx=".78" cy=".82" r=".60">
      <stop offset="0" stop-color="{SHADOW_DEEP}" stop-opacity=".22"/>
    ...
    <linearGradient id="lineRimGrad" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="{LINE_RIM}" stop-opacity=".75"/>
      <stop offset="1" stop-color="{LINE_BOT}" stop-opacity=".15"/>
    </linearGradient>
  ...
  <g clip-path="url(#tileClip)">
    ...
    <g id="bg">
      ...
      <g filter="url(#emberGlowFilter)" opacity=".36">
        {rr(MX + 10, MY + 20, MW - 20, MH - 20, MR)} fill="{EMBER_MID}"/>
      </g>

      <!-- Contact shadow of lower measuring plate on porcelain -->
      <g filter="url(#plateShadow)" opacity=".38">
        {rr(MX + 8, MY + 24, MW, MH, MR)} fill="{SHADOW_DEEP}"/>
      </g>
    </g>
    ...
    <g id="mid">
      ...
      <g clip-path="url(#plateClip)">
        ...
        <g stroke="{EMBER_RULE}" opacity=".46">
          {grid_lines}
        </g>

        <!-- Ruler graduations and tick marks -->
        <g stroke="{EMBER_RULE}" stroke-linecap="round" opacity=".88">
          {top_ticks}
          {left_ticks}
        </g>
        <!-- Highlighted tick catches (light catching the engraved marks) -->
        <g stroke="{EMBER_CORE}" stroke-linecap="round" opacity=".48" transform="translate(0, 1)">
          {top_ticks}
          {left_ticks}
        </g>
      </g>
      ...
      <g filter="url(#sheetShadow)" opacity=".38">
        {rr(SX - 8, SY + 18, SW, SH, SR)} fill="{SHADOW_DEEP}"/>
      </g>
    </g>
    ...
    <g id="fg">
      ...
      <g clip-path="url(#sheetClip)">
        ...
        <g opacity=".42" filter="url(#throughFrostBlur)">
          {measuring_plate} fill="none" stroke="{EMBER_MID}" stroke-width="4"/>
          <g stroke="{EMBER_RULE}" stroke-linecap="round" opacity=".60">
            {top_ticks}
            {left_ticks}
          </g>
          <g stroke="{EMBER_RULE}" opacity=".32">
            {grid_lines}
          </g>
        </g>
      ...
      <g>
        ...
        <g filter="url(#lineEmboss)" opacity=".32">
        ...
        {"".join(f'<path d="M{x + h*.5:.1f} {y + 2:.1f} H{x + w - h*.5:.1f}" stroke="{RIM}" stroke-width="2.5" stroke-linecap="round" opacity=".55"/>' for x, y, w, h in PROSE_LINES)}
        <!-- Highlight catch on semicolon dot and comma head -->
        <circle cx="{SEMI_X:.1f}" cy="{SEMI_Y - 24 * .38 - 1:.1f}" r="3" fill="{RIM}" opacity=".45"/>
        <circle cx="{SEMI_X:.1f}" cy="{SEMI_Y + 24 * .22 - 1:.1f}" r="3" fill="{RIM}" opacity=".40"/>
      </g>
    </g>
    ...
    <g id="highlight">
      ...
      <path d="M{SX + SR} {SY + 2.5} H{SX + SW - SR}" stroke="{RIM}" stroke-width="5"
            stroke-linecap="round" opacity=".85"/>
      <!-- Soft left edge highlight on frosted sheet -->
      <path d="M{SX + 2.5} {SY + SR} V{SY + SH - SR}" stroke="{RIM}" stroke-width="4"
      ...
      <path d="{d}" fill="none" stroke="{RIM}" stroke-width="7" opacity=".84"/>
    </g>
  </g>
</svg>
"""
