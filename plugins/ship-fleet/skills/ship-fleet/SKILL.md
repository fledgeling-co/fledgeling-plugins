...
report and artifacts; retry sharper, resume in-worktree, or park with a reason. Discovered
children join the DAG. At the end: every item `Done` / parked-with-reason, statuses final,
hierarchy refreshed, needs-input items presented as one consolidated list (attended) or left as
`Needs More Info` parks with Easy-reply blocks (unattended).
