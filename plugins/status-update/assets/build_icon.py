#!/usr/bin/env python3
"""build_icon.py — the `status-update` icon: "The Flap Mid-Turn".
...
FLAP_LEAD_Y = 718.0                      # its leading edge, 116 nearer the viewer
FLAP_OVERHANG = 30.0                     # past the board's edges, both sides
...
def build() -> str:
    ...
    # Hinge axle pin notches: diegetic mechanical pivots where the flap mounts.
    # Left notch sits in shadow on the front face; right notch is on the right edge.
    a(f'<circle cx="{RX0 - 12:.1f}" cy="{HINGE_Y:.1f}" r="7" fill="{G_WELL}"/>')
    a(f'<circle cx="{RX0 - 12:.1f}" cy="{HINGE_Y:.1f}" r="4" fill="{G_TOP}"/>')
    a(f'<circle cx="{BX1 + EDX * 0.45:.1f}" cy="{HINGE_Y + EDY * 0.45:.1f}" r="7" fill="{G_DEEP}"/>')
    a(f'<circle cx="{BX1 + EDX * 0.45:.1f}" cy="{HINGE_Y + EDY * 0.45:.1f}" r="4" fill="{G_SHADE}"/>')
    a("</g>")
