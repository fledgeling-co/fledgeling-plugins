...
1. **Enumerate — the inclusion rule is ownership, then activity.** A `~/Dev` dir belongs in the manifest when it is **user-owned**: it has no git repo, no git remote, or its origin owner is one of `lprhodes`, `fledgeling-co`, `elysee-dubois`, `bella*`, `Diolog26`, `DiologIR` (the list may grow — use judgement for a repo that is clearly the user's authorship under a new org, and confirm the new owner with the user before adding it to this list). Repos with third-party origins are excluded from the manifest entirely — not listed, never modified. Then apply the activity window (default 45 days): last commit date for git repos, newest file mtime for non-git dirs. Skip worktree copies (`*-wt`, `*-worktrees`, `*-wave-*` dirs).
