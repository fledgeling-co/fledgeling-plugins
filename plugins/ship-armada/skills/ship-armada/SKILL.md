...
- **A project is complete when its ledger says so, never when its dispatch returns.** A fleet whose runners died reports `completed` — Claude Code's workflow `agent()` returns `null` on a terminal API error with zero retries, and the run still finishes clean. So before you tick a project off, read its `ORCHESTRATOR.md` and confirm every item is `merged` or `parked-with-reason`, and check `git log` on the integration branch for the merges it claims. Ticking on the dispatch return is how a portfolio survey reports a backlog shipped that nothing touched, and in daemon mode there is no one watching to notice. The scheduler-level rule is in ship-fleet's `references/scheduling-and-concurrency.md`.
