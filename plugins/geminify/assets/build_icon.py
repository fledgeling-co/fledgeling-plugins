...
LEAF_W, LEAF_H, LEAF_WALL = 330, 600, 0   # H fitted to the reference's 71%-tall union
LEAF_R = LEAF_W / 2                                    # full capsule ends
# Geometry solved numerically rather than by eye, twice, because the swing is not
# intuitive: rotating about a low shared pivot moves each capsule's TOP outward by
# roughly (pivot_y - top) * sin(lean), so rounds r01-r04 kept producing a pair 74
# to 87% of the tile wide while the lens between them collapsed to a slot.
#
# The second search fitted the RASTER's own measured geometry instead of a guess.
# Sampled off the reference: its capsules are ~300px wide, they are DISJOINT
# across the upper third (left 219-510, right 517-836, a 7px gap at y=315) and
# merge below it, the union is 60% of the tile wide, and the crossing takes about
# a fifth of the union's area. Two capsules that overlap all the way up cannot
# read as a V no matter how far they lean - the gap at the top is the whole tell.
# Fitting (width, height, overlap, lean, pivot) to those four numbers gives:
OVERLAP = 280                                          # pre-rotation, at the base
L_X = 512 - (LEAF_W + (LEAF_W - OVERLAP)) / 2          # 322
R_X = L_X + LEAF_W - OVERLAP                           # 372
L_Y, R_Y = 218, 196                                    # right leaf rides higher:
                                                       # the newer file sits on top
LEAN = 18                                              # left -LEAN, right +LEAN
...
PIVOT_Y = 780                                          # the shared footing both
                                                       # leaves turn about, low and on
                                                       # the centre line, so the pair
                                                       # splays into a V rather than
                                                       # tilting as two separate cards
RULES = [(506, 58, -3.0), (578, 49, 2.5), (650, 40, -1.5)]   # y, half-length, tilt
RULE_H = 16
...
FACE_L = ("#BEB098", "#655843")                        # clay, in shade
...
FACE_R = ("#F5B583", "#B15A22")
WALL_R = ("#A8562A", "#79380F")   # unused while LEAF_WALL == 0

SCATTER = "#FFFBF2"                                    # rim scatter: lighter AND
SCATTER_A, SCATTER_W = 0.58, 14                        # less saturated than the face
SCATTER_R, SCATTER_R_A = "#FFE2C8", 0.80               # warm leaf gets a warm rim
CROWN_A = 0.44                                         # the broad soft highlight
CROWN_H = 0.42                                         # arc inside the upper edge:
                                                       # a poured body is lit across
                                                       # its whole crown, not along a
                                                       # hairline at the outline
...
LENS = ("#8A3F1C", "#5E240C")
LENS_A = 0.55
# The blend zone's OWN edge catch. Measured against the raster take, this is the
# single largest missing feature: the almond there is outlined in warm light all
# the way round, which is what makes two flat overlapping fills read as two
# translucent bodies crossing. Authored as a stroke on each leaf's outline
# clipped to the intersection, so it can only ever land on the real boundary.
LENS_RIM, LENS_RIM_A, LENS_RIM_W = "#FFC79A", 0.72, 11
...
CORE, CORE_A = "#FF7A33", 0.78
BLOOM, BLOOM_A = "#F9C48E", 0.42
...
class Leaf:
    ...
    @property
    def spin(self):
        # BOTH leaves turn about the SAME point, low and on the tile's centre line.
        # Rotating each about its own base (r01) tilts them without splaying: the
        # two pivots are 200px apart, so the tops swing outward independently and
        # the pair reads squat with a narrow slot between them. One shared footing
        # is what makes it a V.
        return f'rotate({self.lean:.2f} {RULE_CX} {PIVOT_Y:.1f})'
        ...
        return [
            ...
            lin(f"rimGrad{self.k}", self.x, self.y, self.x + self.w,
                ...
                [(0, self.scatter, str(self.scatter_a)),
                 ...
                 (0.62, self.scatter, "0")]),
            lin(f"crown{self.k}", self.x, self.y, self.x, self.y + self.h * CROWN_H,
                [(0, self.scatter, str(CROWN_A)),
                 (0.45, self.scatter, str(round(CROWN_A * 0.42, 3))),
                 (1, self.scatter, "0")]),
        ]
    ...
    def crown(self):
        """The broad highlight across the body's upper third. Drawn UNDER nothing and
        clipped to the leaf, so the lit crown spills to the outline rather than
        sitting on it - a hairline at the edge reads as a sticker rim, and a poured
        body is lit across its whole crown."""
        return [
            f'<g transform="{self.spin}" clip-path="url(#leaf{self.k})">',
            f'<rect x="{self.x:.1f}" y="{self.y:.1f}" width="{self.w:.1f}" '
            f'height="{self.h * CROWN_H:.1f}" fill="url(#crown{self.k})" '
            f'filter="url(#soft)"/>',
            '</g>',
        ]
    ...
    def outline(self, stroke, width, opacity):
        """This leaf's own boundary as a stroke, for whoever wants to clip it."""
        return (f'<g transform="{self.spin}">'
                f'<rect x="{self.x:.1f}" y="{self.y:.1f}" width="{self.w:.1f}" '
                f'height="{self.h:.1f}" rx="{self.r}" fill="none" stroke="{stroke}" '
                f'stroke-width="{width}" opacity="{opacity}"/></g>')
...
def lens_rim():
    """The almond's own lit boundary — each leaf's outline, clipped to the other's
    interior, so the catch can only ever land on the real intersection edge. This
    is the feature that separates two overlapping fills from two crossing bodies,
    and it was the largest single difference against the raster take."""
    return [
        f'<g clip-path="url(#leafr)">',
        LEFT.outline(LENS_RIM, LENS_RIM_W, LENS_RIM_A),
        '</g>',
        f'<g clip-path="url(#leafl)">',
        RIGHT.outline(LENS_RIM, LENS_RIM_W, LENS_RIM_A),
        '</g>',
    ]
...
def build():
    ...
    mid = LEFT.shadow() + LEFT.body() + LEFT.crown() + RIGHT.shadow()
    fg = RIGHT.body() + RIGHT.crown() + lens()
    hi = LEFT.rim() + RIGHT.rim() + lens_rim() + bloom()
