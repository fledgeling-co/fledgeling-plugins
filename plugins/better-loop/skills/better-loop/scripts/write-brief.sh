#!/usr/bin/env bash
# better-loop — render docs/loops/loop-<slug>.md into .claude/loop.md,
# size-checking against the 25,000-byte cap.
#
#   write-brief.sh --slug <slug> --from <path> [--dry-run]
#
# Only needed when composing with the built-in /loop: the watcher reads its
# probe from .claude/loops/<slug>.json and never reads loop.md.
#
...
[ -n "$SLUG" ] || { echo "write-brief.sh: --slug is required" >&2; exit 2; }
[ -n "$FROM" ] && [ -f "$FROM" ] || { echo "write-brief.sh: --from <file> is required" >&2; exit 2; }
