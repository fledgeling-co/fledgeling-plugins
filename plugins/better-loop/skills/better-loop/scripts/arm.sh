...
#
# There are no settings to change and no cron to schedule: the loop is a Monitor
# in this session. Arming writes one state file and prints the call to make.

set -uo pipefail
SLUG=""; PROBE=""; STOP_WHEN=""; TICK_CMD=""; BRIEF=""
INTERVAL=120; MAX_WAKES=12; REPEAT_AFTER=1800; DRY_STOP=0; DRY=0
...
NEW="$(jq -n --arg s "$SLUG" --arg p "$PROBE" --arg sw "$STOP_WHEN" --arg tc "$TICK_CMD" \
  ...
  { slug:$s, armed:false, cwd:$c, brief:$b, ledger:$l, probe:$p,
