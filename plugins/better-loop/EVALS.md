...
[`better-goal/EVALS.md`](../better-goal/EVALS.md): each case run twice from an
...
> These numbers were produced by version 1.x, under the former name, and
> before the rebuild around self-armed mechanisms. What they measure is the
> reasoning about `/goal` and `/loop`, which the current skill still carries;
> the guard, the watcher and the change gate are not in them.
...
Combined with `better-goal`: **42/43 assertions against 19/43, and 10/10
preferences.**
...
skill arm for naming the mechanism error outright and routing to `better-goal`,
...
such script in `better-loop`, and reported it as a skill gap rather than
improvising. That name belongs to `better-goal`. Step 4 now points at
