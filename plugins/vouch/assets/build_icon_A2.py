#!/usr/bin/env python3
"""build_icon_A2.py — Engine A, second take: "The Tie-Back, portrait".
...
This take exists to test the axis the raster engine raised: a PORTRAIT document
with a TAPERED filament, against take A's landscape page and constant-width rod.
The raster's needle reads beautifully at 1024 and disappears by 32px, so the
question the sheet has to answer is whether a taper can keep enough mass at the
landing point to survive the squint. It cannot, and that is the finding.
...
ROW_X, ROW_Y, ROW_W, ROW_H = 262, 196, 500, 92     # the ledger row
DOC_X, DOC_Y, DOC_W, DOC_H = 336, 452, 352, 420     # the source document
DOC_FOLD   = 74                                      # folded corner, top-right
THREAD_X   = 512                                     # the filament, dead centre
THREAD_TOP = ROW_Y + ROW_H
THREAD_BOT = DOC_Y + 176                              # it lands INSIDE the page
THREAD_W   = 14   # the taper the raster used; the sheet records what it costs                                      # survives 16px: 0.41px at 16
BLOOM_R    = 150
...
def build() -> str:
    ...
    for i in range(4):
        a(f'<rect x="{DOC_X+52}" y="{DOC_Y+232+i*40}" width="{[236,196,236,160][i]}" height="10" rx="5" '
    ...
    a(f'<path d="M{THREAD_X - THREAD_W},{THREAD_TOP - 10} '
      f'L{THREAD_X + THREAD_W},{THREAD_TOP - 10} '
      f'L{THREAD_X + THREAD_W*0.36:.1f},{THREAD_BOT} '
      f'L{THREAD_X - THREAD_W*0.36:.1f},{THREAD_BOT} z" fill="url(#thread)"/>')
    a(f'<circle cx="{THREAD_X}" cy="{THREAD_BOT}" r="{THREAD_W*1.6:.0f}" fill="{EMBER_CORE}"/>')
