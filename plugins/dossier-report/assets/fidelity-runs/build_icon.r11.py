...
FLAP_FREE = "#F46F41"            # the free-edge end, most open to the sky
FLAP_MID = "#E85128"             # the plane's own tone
FLAP_DEEP = "#CE3A16"            # toward the crease, where the V closes
...
SPINE_CREST = "#FFE7D6"          # r11: the spine crest, warm. Never #FFFFFF: see the highlight pass.
SPINE_CREST_OPACITY = 0.30
...
def build():
    ...
    mid = [
        ...
        f'<g filter="url(#bXL)" opacity="0.15"><path d="{poly(shift(sil, 8, 40))}" fill="{SHADOW}"/></g>',
        f'<g filter="url(#bS)" opacity="0.26"><path d="{poly(shift(sil, 5, 22))}" fill="{SHADOW}"/></g>',
    ...
    for arc in lit_arc(base_poly, False):
        hl.append(f'<g clip-path="url(#pageClip)"><path d="{poly(arc, close=False)}" fill="none" '
                  ...
                  f'stroke-linecap="round" filter="url(#bM)"/></g>')
    hl.append(
        # The crest catches the key along the spine. r11: this was a pure-white
        # hairline at 0.55 through the finest blur, and the blind panel read it as
        # "a hard specular streak ... a gloss decal rather than volume" - Tahoe
        # grammar #6 sanctions one soft top light and zero hard speculars, and
        # nothing in this scene emits white light. Warmed to the measured rim
        # scatter, halved in strength, widened, and pushed through the medium blur
        # so it reads as a rolled edge turning into the key.
        f'<g clip-path="url(#flapClip)"><path d="{poly(offset_poly([A, B], 0, nx, ny) + offset_poly([B, A], 9, nx, ny))}" '
        f'fill="{SPINE_CREST}" fill-opacity="{SPINE_CREST_OPACITY}" filter="url(#bM)"/></g>')
