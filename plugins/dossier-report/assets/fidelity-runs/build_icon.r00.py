...
PAGE_MID = "#F9F5EE"
PAGE_LOW = "#EFE9DE"
PAGE_EDGE_LIT = "#FFFFFF"
PAGE_EDGE_LOW = "#E4DCCD"
...
FLAP_FREE = "#F2663A"            # L 0.50 - the free-edge end, most open to the sky
FLAP_MID = "#E5471E"             # L 0.40
FLAP_DEEP = "#C42D0D"            # L 0.29 - the crease corner, mutually occluded
...
def build():
    ...
    def lit_arc(pts, lo=True):
        out, run = [], []
        for i in range(len(pts)):
            p, q = pts[i], pts[(i + 1) % len(pts)]
            ex, ey = q[0] - p[0], q[1] - p[1]
            outward = (ey, -ex)          # right-hand normal on a CW outline
            facing = outward[0] * KEY[0] + outward[1] * KEY[1]
            if (facing > 0) == lo:
                run.append(p)
            elif run:
                out.append(run + [p])
    ...
    g.append(f'''<linearGradient id="pageEdge" x1="{PAGE_X0}" y1="{PAGE_Y1}" x2="{PAGE_X1}" y2="{PAGE_Y0}" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="{PAGE_EDGE_LIT}"/><stop offset="0.62" stop-color="#F3EDE1"/>
    ...
    mid = [
        # the sheet's contact shadow on the porcelain: two radii, both warm
    ...
    fg = [
        # the slab's thickness along the flap's real edges - never the crease
        f'<path d="{poly(flap_edge)}" fill="url(#flapEdge)"/>',
        # the inner face
        f'<path d="{poly(flap_poly)}" fill="url(#flapFace)"/>',
        f'<g clip-path="url(#flapClip)"><path d="{poly(flap_poly)}" fill="url(#flapAcross)"/></g>',
        # the V's own occlusion, hugging the crease from inside the flap
        f'<g clip-path="url(#flapClip)"><g filter="url(#bM)" opacity="{AO_CREASE_OPACITY}">'
        f'<path d="{poly(offset_poly([A, B], -40, nx, ny) + offset_poly([B, A], 4, nx, ny))}" fill="{FLAP_AO}"/></g></g>',
