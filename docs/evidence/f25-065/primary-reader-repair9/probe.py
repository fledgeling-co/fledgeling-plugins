...
assert boundaries["trailingHelperInvokesAfterMutation"]["cliExit"] == 0
...
# Material false green: helper closure invocation occurs before the scoped mutation.
assert boundaries["trailingHelperInvokesBeforeMutation"]["cliExit"] == 0
...
mutants = [
    ...
    run_mutant(
        ...
        b"                        False):\n",
        lambda module: observe_case_invalid(module, "readerInsideHelperTrailingClosure", "seed {"),
    ),
    run_mutant(
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "A helper's direct closure invocation can occur before the scoped mutation, so invocation anywhere in the helper body does not prove a later observation.",
        "source": sources["trailingHelperInvokesBeforeMutation"][0],
        "cliExit": boundaries["trailingHelperInvokesBeforeMutation"]["cliExit"],
    },
