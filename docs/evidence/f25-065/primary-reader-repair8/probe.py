...
# Material false green: an @autoclosure argument is stored unless the helper invokes it.
assert boundaries["autoclosureReaderArgument"]["cliExit"] == 0
...
mutants = [
    ...
    run_mutant(
        "allow-helper-trailing-closure",
        b'                pattern = re.escape(str(row.get("name"))) + r"\\s*\\("\n',
        b'                pattern = re.escape(str(row.get("name"))) + r"\\s*(?:\\(|\\{)"\n',
        observe_helper_trailing_closure,
    ),
    run_mutant(
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "A parenthesized helper can receive a reader call as an unevaluated @autoclosure argument, so lexical nesting after the opening parenthesis does not prove invocation.",
        "source": sources["autoclosureReaderArgument"][0],
        "cliExit": boundaries["autoclosureReaderArgument"]["cliExit"],
    },
