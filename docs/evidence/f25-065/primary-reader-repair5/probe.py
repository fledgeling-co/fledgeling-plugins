...
TARGET = "1e6ed483a6b9ba4dcfe1590323c7eb7dc6261ca9"
RETAINED_FAILS = (
...
# Material false green: a local computed-property getter is a declaration, not a call.
assert boundaries["localGetterDeclaration"]["cliExit"] == 0
...
def observe_invocation_context(module):
    assert not module.has_reader_call("if read { print(x) }", ("read",)), \
        "control condition accepted after removing invocation context"
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "Accepted trailing-closure shape after an opening brace treats a local computed-property getter declaration as a reader call.",
        "source": sources["localGetterDeclaration"][0],
        "cliExit": boundaries["localGetterDeclaration"]["cliExit"],
    },
