...
TARGET = "68c0c5b74b0d143b8738fc7e1c4283a0e2c89b6c"
LATER = "611bd1151b47f257069e184b7608f582f297200a"
...
assert boundaries["validParenthesized"]["cliExit"] == 0
assert boundaries["validTrailingClosure"]["cliExit"] == 0
...
# Material false green: neither source contains an executable configured reader call.
assert boundaries["identifierSubstringOnly"]["cliExit"] == 0
assert boundaries["bareReaderNameOnly"]["cliExit"] == 0


base_source = sources["validParenthesized"][0]
...
mutants = [
    run_mutant(
        "suppress-whole-body-after-one-scope",
        b"            calls = [call for call in calls if call not in ignored]\n",
        b"            calls = [] if ignored else calls\n",
        observe_exact_recheck,
    ),
    run_mutant(
        "accept-comment-only-caller-spelling",
        b"executable_match = (re.match(pattern, masked_caller[offset:])",
        b"executable_match = (re.match(pattern, caller_body[offset:])",
        observe_executable_caller,
    ),
    run_mutant(
        "ignore-target-test-entry-posture",
        b"                           row.get(\"testEntry\") == test_entry]\n",
        b"                           True]\n",
        observe_posture,
    ),
    run_mutant(
        "globally-suppress-four-verbs",
        b"            for v in mutators:\n",
        b"            for v in (v for v in mutators if v not in {\"record\", \"store\", \"apply\", \"update\"}):\n",
        observe_no_global_suppression,
    ),
]

later_paths = git("diff", "--name-only", TARGET, LATER).decode().splitlines()
assert later_paths == [
    "docs/evidence/f25-065/additional/catalogue.log",
    "docs/evidence/f25-065/additional/focused-1.log",
    "docs/evidence/f25-065/additional/focused-2.log",
    "docs/evidence/f25-065/additional/full-1.log",
    "docs/evidence/f25-065/additional/full-2.log",
    "docs/evidence/f25-065/additional/probe.json",
    "docs/evidence/f25-065/additional/probe.py",
    "docs/evidence/f25-065/additional/receipt.json",
    "docs/evidence/f25-065/additional/verification.md",
]

receipt = {
    "verdict": "FAIL",
    "reviewedHead": TARGET,
    "laterEvidenceOnlyHead": LATER,
    "sourceSHA256": source_hash,
    "blockingFalseGreen": {
        "claim": "Attributed-helper reader validation accepts identifiers and bare names rather than an executable configured reader call.",
        "identifierSubstringCLIExit": boundaries["identifierSubstringOnly"]["cliExit"],
        "bareReaderNameCLIExit": boundaries["bareReaderNameOnly"]["cliExit"],
    },
    "boundaries": boundaries,
    "strictSchemaCases": schema_cases,
    "actualSourceMutants": mutants,
    "laterCommitChangedPaths": later_paths,
