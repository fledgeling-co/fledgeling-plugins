...
mutants=[
 ...
 ('allow-return-position-helper',simple(
  ...
  '    if False and re.search(r"\\b(?:return|throw)\\b", source[:start]):')),
 ('narrow-helper-terminal-to-current-statement',simple(
  '    if re.search(r"\\b(?:return|throw)\\b", source[:start]):',
...
try:
 for i,(name,mutate) in enumerate(mutants):
  ...
  with tempfile.TemporaryDirectory(prefix=f'f25-065-primary14-mut-{i}-') as cache:
