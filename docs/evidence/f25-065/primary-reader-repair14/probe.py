...
spec = importlib.util.spec_from_file_location('f25065_primary14', SCRIPT)
...
def cli(source: str, helper_call: str = 'seed(') -> dict:
    with tempfile.TemporaryDirectory(prefix='f25-065-primary14-') as td:
...
cases={
 ...
 'intentional-refusal-after-false-if-return':('private func seed() { store() }\nfunc testMeasure() { if false { return }; seed(); read() }','seed(',1),
 'valid-semicolon-boundary':('private func seed() { store() }\nfunc testMeasure() { let value = 1; seed(); read(); _ = value }','seed(',0),
 'valid-comment-terminal-word':('private func seed() { store() }\nfunc testMeasure() { /* return throw */ seed(); read() }','seed(',0),
 'valid-string-terminal-word':('private func seed() { store() }\nfunc testMeasure() { let note = "return throw"; seed(); read(); _ = note }','seed(',0),
 'intentional-refusal-after-stored-closure-return':('private func seed() { store() }\nfunc testMeasure() { let closure = { return }; _ = closure; seed(); read() }','seed(',1),
 'intentional-refusal-after-escaped-return-call':('private func seed() { store() }\nfunc testMeasure() { `return`(); seed(); read() }\nfunc `return`() {}','seed(',1),
