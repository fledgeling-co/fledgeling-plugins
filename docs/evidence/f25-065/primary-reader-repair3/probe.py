...
TARGET = "08007d341f2bac53b6d19d7b0dd9087a8e427085"
RETAINED_FAILS = (
...
# Material false green: #selector references but does not invoke a reader.
assert boundaries["selectorReaderReference"]["cliExit"] == 0
...
mutants = [
    ...
    run_mutant(
        "allow-qualified-reader-shape",
        b'        r"(?<![.\\w`])" + re.escape(reader) + r"\\w*\\s*\\(", source\n',
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*\\s*\\(", source\n',
        observe_qualified_pattern,
    ),
    run_mutant(
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "Parenthesized reader syntax also matches a #selector method reference and accepts a caller with no reader invocation.",
        "source": sources["selectorReaderReference"][0],
        "cliExit": boundaries["selectorReaderReference"]["cliExit"],
    },
