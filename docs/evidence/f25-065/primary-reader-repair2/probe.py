...
TARGET = "35d7159d6354cb776bf5a7ec1e5de67315c6388a"
RETAINED_FAILS = (
...
# Material false green: an enum associated-value pattern is not a reader call.
assert boundaries["enumReaderPattern"]["cliExit"] == 0
...
mutants = [
    ...
    run_mutant(
        "allow-ambiguous-reader-trailing-shape",
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*\\s*\\(", source\n',
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*\\s*(?:\\(|\\{)", source\n',
        observe_control_condition,
    ),
    run_mutant(
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "Parenthesized reader syntax also matches an enum associated-value pattern and accepts a caller with no reader invocation.",
        "source": sources["enumReaderPattern"][0],
        "cliExit": boundaries["enumReaderPattern"]["cliExit"],
    },
