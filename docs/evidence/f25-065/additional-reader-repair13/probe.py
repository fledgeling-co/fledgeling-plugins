#!/usr/bin/env python3
"""Distinct additional probes for F25-065 multiline-terminal repair b9366d9."""
...
TARGET = "b9366d9ea6f282610dc64760a9f38586a8144e8e"
...
exact_root = Path(tempfile.mkdtemp(prefix="f25-065-additional-r13-exact-"))
...
def direct(source: str, helper_call: str = "seed(") -> dict:
    with tempfile.TemporaryDirectory(prefix="f25-065-additional-r13-direct-") as directory:
        root = Path(directory)
...
def public_cli(source: str, helper_call: str = "seed(") -> dict:
    with tempfile.TemporaryDirectory(prefix="f25-065-additional-r13-cli-") as directory:
        root = Path(directory)
        tests = root / "tests"
        tests.mkdir()
...
cases = {
    ...
    "returnLineCommentNewline": (
        "private func seed() { store() }\n"
        "@Test func measure() { return // ends the return statement\n seed(); read() }", "seed("),
    "throwSameLine": (
    ...
    "semicolonReturnThenHelper": (
        "private func seed() { store() }\n"
        "@Test func measure() { return; seed(); read() }", "seed("),
...
assert observed["returnNewline"]["publicCLI"]["exit"] == 1
assert observed["returnBlockCommentNewline"]["publicCLI"]["exit"] == 1
assert observed["returnLineCommentNewline"]["publicCLI"]["exit"] == 1
assert observed["throwSameLine"]["publicCLI"]["exit"] == 1
assert observed["throwNewline"]["publicCLI"]["exit"] == 1
assert observed["semicolonReturnThenHelper"]["publicCLI"]["exit"] == 1
assert observed["unconditionalDoReturnThenHelper"]["publicCLI"]["exit"] == 0
assert observed["unconditionalDoThrowThenHelper"]["publicCLI"]["exit"] == 0
assert observed["repeatReturnThenHelper"]["publicCLI"]["exit"] == 0
assert observed["nonterminatingDoThenHelper"]["publicCLI"]["exit"] == 0
...
result = {
    ...
    "blocker": (
        "A top-level helper after an unconditional do/repeat block is accepted even when a return or throw in that always-executed block makes both the helper and later reader unreachable."
    ),
    "cases": observed,
