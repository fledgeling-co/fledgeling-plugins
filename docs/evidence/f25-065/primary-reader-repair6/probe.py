...
# Material false green: Swift labels are Unicode identifiers, not ASCII-only.
assert boundaries["unicodeLabelReaderFunctionReference"]["cliExit"] == 0
...
def observe_reader_trailing_closure(module):
    assert not module.has_reader_call("var value: Int { get { 1 } }", ("get",)), \
        "accessor accepted after restoring reader trailing-closure shape"
...
receipt = {
    ...
    "blockingFalseGreen": {
        "claim": "Function-reference placeholder grammar is ASCII-only and accepts a valid Swift Unicode-label reference without invocation.",
        "source": sources["unicodeLabelReaderFunctionReference"][0],
        "cliExit": boundaries["unicodeLabelReaderFunctionReference"]["cliExit"],
    },
