...
TARGET = "2de5a2e6d0ee8282232e3fd13280653b729ced62"
RETAINED_FAIL = "dd80213bd27aa880b6227aa4b961f649e0d314a0"
...
sources = {
    "qualifiedParenthesizedReader": (
        "private func seed() { store() }\n"
        "@Test func measure() { seed(); Fixtures.read() }",
        "seed(",
    ),
    "qualifiedTrailingClosureReader": (
        "private func seed() { store() }\n"
        "@Test func measure() { seed(); Fixtures.read { configure() } }",
        "seed(",
    ),
    "readerBefore": (
        "private func seed() { store() }\n"
...
assert boundaries["qualifiedParenthesizedReader"]["cliExit"] == 0
assert boundaries["qualifiedTrailingClosureReader"]["cliExit"] == 0
...
# Material false green: a control-flow condition is not a reader invocation.
assert boundaries["readerShapedControlCondition"]["cliExit"] == 0


base_source = sources["qualifiedParenthesizedReader"][0]
...
mutants = [
    run_mutant(
        "drop-reader-identifier-boundary",
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*\\s*(?:\\(|\\{)", source\n',
        b'        r"" + re.escape(reader) + r"\\w*\\s*(?:\\(|\\{)", source\n',
        observe_identifier_boundary,
    ),
    run_mutant(
        "drop-reader-call-syntax",
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*\\s*(?:\\(|\\{)", source\n',
        b'        r"(?<![\\w`])" + re.escape(reader) + r"\\w*", source\n',
        observe_call_syntax,
    ),
    run_mutant(
...
retained_path = "docs/evidence/f25-065/primary-final/verification.md"
retained_fail_unchanged = (git("show", f"{RETAINED_FAIL}:{retained_path}") ==
                           (ROOT / retained_path).read_bytes())
assert retained_fail_unchanged

receipt = {
    "verdict": "FAIL",
    "reviewedHead": TARGET,
    "retainedPriorFailure": RETAINED_FAIL,
    "retainedPriorFailureUnchanged": retained_fail_unchanged,
    "sourceSHA256": source_hash,
    "blockingFalseGreen": {
        "claim": "Trailing-closure reader syntax also matches a Swift control-statement condition and accepts a caller with no reader invocation.",
        "source": sources["readerShapedControlCondition"][0],
        "cliExit": boundaries["readerShapedControlCondition"]["cliExit"],
    },
